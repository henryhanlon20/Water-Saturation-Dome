...
s_data = [12, 8];
figure(1);
scatter(s_data, t_data, 'filled');
hold on;
text(s_data(1), t_data(1), 'State 1', 'FontSize', 12, 'HorizontalAlignment', 'right');
text(s_data(2), t_data(2), 'State 2', 'FontSize', 12, 'HorizontalAlignment', 'right');
xlabel('Entropy [kJ/kg*k]');
ylabel('Temperature [K]');

%Now we want to add our vapor dome to the plot:
[x,y,x0,y0]=domeData("ts",100,.002);
plot(x,y);
scatter(x0,y0);
